...
if __name__ == "__main__":
    ...
    auc_scores, f1_scores, mcc_scores = [], [], []
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X_scaled, y)):
        ...
        fold_f1 = f1_score(y_test, y_pred_bin)
        fold_mcc = matthews_corrcoef(y_test, y_pred_bin)
        ...
        print(f"Fold {fold + 1} AUC: {fold_auc:.4f}, F1: {fold_f1:.4f}, MCC: {fold_mcc:.4f}")

    # 最终评估
    print(f"\n平均AUC: {np.mean(auc_scores):.4f} (±{np.std(auc_scores):.4f})")
    print(f"平均F1-score: {np.mean(f1_scores):.4f} (±{np.std(f1_scores):.4f})")
    print(f"平均MCC: {np.mean(mcc_scores):.4f} (±{np.std(mcc_scores):.4f})")
    ...
    plt.plot(fold_numbers, auc_scores, 'o-', color='darkred', label='单Fold AUC')
    plt.axhline(np.mean(auc_scores), color='navy', linestyle='--',
                label=f'平均 ({np.mean(auc_scores):.3f}±{np.std(auc_scores):.3f})')
    plt.ylim(0.5, 1.0)
    plt.xlabel('Fold序号')
    plt.ylabel('AUC')
    plt.title('AUC交叉验证趋势')
    ...
    # F1趋势
    plt.subplot(1, 3, 2)
    plt.plot(fold_numbers, f1_scores, 'o-', color='forestgreen', label='单Fold F1')
    plt.axhline(np.mean(f1_scores), color='purple', linestyle='--',
                label=f'平均 ({np.mean(f1_scores):.3f}±{np.std(f1_scores):.3f})')
    plt.ylim(0.0, 1.0)
    plt.xlabel('Fold序号')
    plt.ylabel('F1 Score')
    plt.title('F1 Score交叉验证趋势')
    plt.legend()
    plt.grid(alpha=0.3)

    # MCC趋势
    plt.subplot(1, 3, 3)
    plt.plot(fold_numbers, mcc_scores, 'o-', color='royalblue', label='单Fold MCC')
    plt.axhline(np.mean(mcc_scores), color='orange', linestyle='--',
                label=f'平均 ({np.mean(mcc_scores):.3f}±{np.std(mcc_scores):.3f})')
    plt.ylim(-1.0, 1.0)
    plt.xlabel('Fold序号')
    plt.ylabel('MCC')
    plt.title('MCC交叉验证趋势')
    ...
    metrics = ['AUC', 'F1', 'MCC']
    means = [np.mean(auc_scores), np.mean(f1_scores), np.mean(mcc_scores)]
    stds = [np.std(auc_scores), np.std(f1_scores), np.std(mcc_scores)]

    bars = plt.bar(metrics, means, yerr=stds, capsize=10,
                   color=['#1f77b4', '#2ca02c', '#d62728'], alpha=0.7)
