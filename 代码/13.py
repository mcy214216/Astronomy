...
if __name__ == "__main__":
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X_scaled, y)):
        ...
        fold_mcc = matthews_corrcoef(y_test, y_pred_class)
    ...
    print(f"平均 MCC: {np.mean(mcc_scores):.4f} (±{np.std(mcc_scores):.4f})")
    ...
    metrics = [auc_scores, f1_scores, precision_scores, accuracy_scores, mcc_scores]
    metric_names = ['AUC', 'F1-score', 'Precision', 'Accuracy', 'MCC']
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']

    for i, (values, name, color) in enumerate(zip(metrics, metric_names, colors)):
        plt.plot(range(1, 11), values, 'o--', color=color,
